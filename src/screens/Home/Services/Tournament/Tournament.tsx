...
const tournamentDetailData = {
  ...
  description: `We provide end-to-end solutions for organizing professional gaming tournaments — from concept planning to execution

Our services include:
- Tournament structure design (brackets, rules, formats) 
- Live streaming with high-quality overlays and commentary 
- Team coordination and on-site operations  
- Sponsor integration and branding support 
- Post-event highlight reels and recap videos

Whether it’s an online or on-ground event, we deliver competitive energy, seamless production, and unforgettable experiences`,

  image: "/tournament.jpg",
  gallery: [
    "/Tournament/tournament1.jpg",
    "/Tournament/tournament2.jpg",
    "/Tournament/tournament3.jpg",
    "/Tournament/tournament4.jpg",
  ],
};

export const Tournament = (): JSX.Element => {
  const [currentSlide, setCurrentSlide] = useState(0);
  const galleryLength = tournamentDetailData.gallery.length;
    ...
    <div className="min-h-screen bg-white text-black flex flex-col">
      ...
      <header className="relative h-64 md:h-80 overflow-hidden flex items-start justify-center">
        ...
        <div
          ...
          style={{
            backgroundImage: `url(${tournamentDetailData.image})`,
            // ✅ แก้ไข: เพิ่ม backgroundPosition
            backgroundPosition: "center 55%", // ตัวอย่าง: จัดกึ่งกลางแนวนอน, เลื่อนขึ้นไป 40% แนวตั้ง
          }}
        />
        ...
        <div className="container mx-auto px-4 relative z-10 w-full pt-16 md:pt-20">
          <div className="text-sm text-white font-medium">
            ...
            <span className="font-semibold">{tournamentDetailData.title}</span>
          </div>
        </div>
      </header>
        ...
        <main className="container mx-auto px-4 py-10 flex-grow max-w-4xl">
          ...
          <div className="relative mb-14 w-full max-w-4xl mx-auto">
            <div className="overflow-hidden rounded-xl shadow-lg relative">
              {/* รูปภาพหลักที่แสดง */}
              <img
                src={tournamentDetailData.gallery[currentSlide]}
                alt={`${tournamentDetailData} Slide ${currentSlide + 1}`}
              ...
              <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex space-x-2 z-10">
                {tournamentDetailData.gallery.map((_, index) => (
                  <button
                    key={index}
          ...
          <p className="text-gray-700 text-lg leading-relaxed whitespace-pre-line">
            {tournamentDetailData.description}
          </p>
        </main>
        <Footer />
      </div>
